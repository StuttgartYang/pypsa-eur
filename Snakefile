...
#     output:
#         profile="resources/"+config["energy_year"]+"/profile_{technology}.nc",
...
#         input:
#             country_shapes='resources/country_shapes.geojson',
#             eia_hydro_generation='data/bundle/EIA_hydro_generation_1980_2016.csv',
#             cutout="cutouts/" + config["renewable"]['hydro']['cutout']
#         output: 'resources/'+config["energy_year"]+'/profile_hydro.nc'
...
#         load='data/bundle/time_series_60min_singleindex_filtered.csv',
#         nuts3_shapes='resources/nuts3_shapes.geojson',
# #         **{f"profile_{tech}": f"resources/2014/profile_{tech}.nc"
# #            for tech in config['renewable']}
#         **{'profile_' + tech: "resources/"+config["energy_year"]+"/profile_" + tech + ".nc"
#            for tech in config['renewable']}
#     output: "networks/"+config["energy_year"]+"/elec.nc"
...
#     input:
#         network='networks/'+config["energy_year"]+'/elec.nc',
#         tech_costs=COSTS,
#         regions_onshore="resources/regions_onshore.geojson",
#         regions_offshore="resources/regions_offshore.geojson"
#     output:
#         network='networks/'+config["energy_year"]+'/elec_s{simpl}.nc',
#         regions_onshore="resources/"+config["energy_year"]+"/regions_onshore_elec_s{simpl}.geojson",
#         regions_offshore="resources/"+config["energy_year"]+"/regions_offshore_elec_s{simpl}.geojson",
...
#         tech_costs=COSTS
#     output:
#         network='networks/'+config["energy_year"]+'/elec_s{simpl}_{clusters}.nc',
#         regions_onshore="resources/"+config["energy_year"]+"/regions_onshore_elec_s{simpl}_{clusters}.geojson",
#         regions_offshore="resources/"+config["energy_year"]+"/regions_offshore_elec_s{simpl}_{clusters}.geojson",
...
#         network='networks/'+config["energy_year"]+'/elec_s{simpl}_{clusters}.nc',
#         tech_costs=COSTS,
#     output: 'networks/'+config["energy_year"]+'/elec_s{simpl}_{clusters}_ec.nc'
...
rule build_robust_capacities:
    input:
        unprepared="networks/"+config["energy_year"]+"/elec_s{simpl}_{clusters}_ec_l{ll}_{opts}_storage_units.nc",
        #optimized="results/networks/"+config["energy_year"]+"/elec_s{simpl}_{clusters}_ec_l{ll}_{opts}.nc"
        #optimized="results/networks/2012/elec_s{simpl}_{clusters}_ec_l{ll}_{opts}_op.nc"
    output: "results/networks/"+config["energy_year"]+"/robust_capacities/elec_s{simpl}_{clusters}_ec_l{ll}_{opts}.nc"
    log:
        solver=normpath("logs/build_robust_capacities/elec_s{simpl}_{clusters}_ec_l{ll}_{opts}_op_solver.log"),
        python="logs/build_robust_capacities/elec_s{simpl}_{clusters}_ec_l{ll}_{opts}_op_python.log",
        memory="logs/build_robust_capacities/elec_s{simpl}_{clusters}_ec_l{ll}_{opts}_op_memory.log"
    benchmark: "benchmarks/build_robust_capacities/elec_s{simpl}_{clusters}_ec_l{ll}_{opts}"
    threads: 4
    resources: mem=(lambda w: 5000 + 372 * int(w.clusters))
    shadow: "shallow"
